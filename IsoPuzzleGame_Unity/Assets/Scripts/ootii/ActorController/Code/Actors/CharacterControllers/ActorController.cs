...
namespace com.ootii.Actors
{
    ...
    public class ActorController : BaseSystemController, ICharacterController
    {
        ...
        /// </summary>
        protected float mSkippedFixedUpdates = 0f;
        ...
        { 
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (_ExtrapolatePhysics && mSkippedFixedUpdates > 0f)
                    {
                        mBorrowedFixedMovement = mAccumulatedVelocity * (rDeltaTime / (mSkippedFixedUpdates + 1f));
                ...
                if (mOnPreControllerMove != null)
                {
                    ...
                    mState.Velocity = mState.Position - mPrevState.Position;
                ...
                //DONT HANDLE ROTATION FROM CONTROLLER
        ...
        protected bool TestGrounding(Vector3 rPosition, Vector3 rActorUp, Vector3 rWorldUp, out RaycastHit rGroundHitInfo)
        {
            ...
            {
                lGroundHit = RaycastExt.SafeRaycast(lRayStart, lRayDirection, lRayDistance, _GroundingLayers, _Transform, out rGroundHitInfo);
            }
            else
            {
                lGroundHit = RaycastExt.SafeRaycast(lRayStart, lRayDirection, lRayDistance, _Transform, out rGroundHitInfo);
            }
        ...
        protected bool TestSlope(Vector3 rPosition, Vector3 rActorUp, Vector3 rMovement, float rCurrentGroundSurfaceAngle, ref Vector3 rSafeMovement, ref Vector3 rGroundNormal)
        {
            ...
            if (_IsGroundingLayersEnabled)
            {
                lGroundHit = RaycastExt.SafeRaycast(rPosition + (rActorUp * EPSILON), lMovementDirection, lMovementDistance + _SkinWidth, _GroundingLayers, _Transform, out lHitInfo);
            }
            else
            {
                lGroundHit = RaycastExt.SafeRaycast(rPosition + (rActorUp * EPSILON), lMovementDirection, lMovementDistance + _SkinWidth, _Transform, out lHitInfo);
            }
            ...
            if (lLocalHitPoint.y < _MaxStepHeight)
            {
                ...
                if (_IsGroundingLayersEnabled)
                {
                    lGroundHit = RaycastExt.SafeRaycast(rPosition + (rActorUp * _MaxStepHeight), lMovementDirection, 1.5f, _GroundingLayers, _Transform, out lStepHitInfo);
                }
                else
                {
                    lGroundHit = RaycastExt.SafeRaycast(rPosition + (rActorUp * _MaxStepHeight), lMovementDirection, 1.5f, _Transform, out lStepHitInfo);
                }
            ...
            while (lDistanceSqr > EPSILON_SQR && lDistanceSqr >= lMinDistanceSqr)
            {
                ...
                if (_IsGroundingLayersEnabled)
                {
                    lGroundHit = RaycastExt.SafeRaycast(lMid + (rActorUp * _MaxStepHeight), -rActorUp, _MaxStepHeight + 0.05f, _GroundingLayers, _Transform, out lHitInfo);
                }
                else
                {
                    lGroundHit = RaycastExt.SafeRaycast(lMid + (rActorUp * _MaxStepHeight), -rActorUp, _MaxStepHeight + 0.05f, _Transform, out lHitInfo);
                }
                ...
                for (int i = 0; i < BodyShapes.Count; i++)
                {
                    ...
                    if (lBodyShapeHits != null)
                    {
                        for (int j = 0; j < lBodyShapeHits.Length; j++)
                        {
                            ...
                            // If we get here, add the shape
                            mBodyShapeHits.Add(lBodyShapeHits[j]);
                        }
                    }
                }
...
//                    EditorApplication.isPaused = true;
        ...
        protected bool ProcessGrounding(Vector3 rActorPosition, Vector3 rOffset, Vector3 rActorUp, Vector3 rWorldUp, float rGroundRadius, out RaycastHit rGroundHitInfo)
        {
            ...
            if (_IsGroundingLayersEnabled)
            {
                lIsGrounded = RaycastExt.SafeRaycast(lRayStart, lRayDirection, lRayDistance, _GroundingLayers, _Transform, out rGroundHitInfo);
            }
            else
            {
                lIsGrounded = RaycastExt.SafeRaycast(lRayStart, lRayDirection, lRayDistance, _Transform, out rGroundHitInfo);
            }
            ...
            if (!lIsGrounded)
            {
                ...
                if (_IsGroundingLayersEnabled)
                {
                    lColliders = RaycastExt.SafeOverlapSphere(lRayStart, rGroundRadius * ONE_OVER_COS45, _GroundingLayers, _Transform);
                }
                else
                {
                    lColliders = RaycastExt.SafeOverlapSphere(lRayStart, rGroundRadius * ONE_OVER_COS45, _Transform);
                }

                // With one or no colliders, this is easy
                if (lColliders == null || lColliders.Length == 0)
                {
                    lIgnore = true;
                    lIsGrounded = false;
                }
                ...
                else
                {
                    ...
                    Vector3[] lOrbitPoints = new Vector3[lColliders.Length];
                    Vector3[] lLocalOrbitPoints = new Vector3[lColliders.Length];
                    float[] lOrbitAngles = new float[lColliders.Length];
                ...
                if (!lIgnore)
                {
                    ...
                    if (_IsGroundingLayersEnabled)
                    {
                        lGroundHit = RaycastExt.SafeRaycast(lRayStart, lRayDirection, lRayDistance, _GroundingLayers, _Transform, out lRaycastHit);
                    }
                    else
                    {
                        lGroundHit = RaycastExt.SafeRaycast(lRayStart, lRayDirection, lRayDistance, _Transform, out lRaycastHit);
                    }
        ...
        public void OnSceneGUI()
        {
            ...
            for (int i = 0; i < BodyShapes.Count; i++)
            {
                ...
                else if (BodyShapes[i] is BodyCapsule)
                {
                    ...
                    Quaternion lRotation = Quaternion.LookRotation(lDirection, _Transform.up);
